...
import type { GameID, RoomID, RoomAudioState, RoomPlaylistPlaybackState } from "@astral-atlas/wildspace-models";
...
const useCurrentTrack = (state, tracks, deps) => {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      id = setTimeout(update, currentTrack.track.trackLengthMs - currentTrack.trackProgress);
    };
...
export const AudioPlaylistStateEditor/*: Component<AudioPlaylistStateEditorProps>*/ = ({
  ...
  return [
    !!currentAsset && h('audio', { src: currentAsset.url, ref: audioRef, controls: true, onSeeked }),
