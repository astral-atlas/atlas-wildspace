...
export const createFileTable = /*::<V>*/(
  filePath/*: string*/,
  castValue/*: Cast<V>*/,
)/*: Table<string, V>*/ => {
  const castTable = createArrayCaster(createObjectCaster({ key: castString, value: castValue }));

  const loadTable = async () => {
    return castTable(JSON.parse(await readFile(filePath, 'utf-8')));
  };

  const get = async (key) => {
    const table = await loadTable();
    const entry = table.find(row => key === row.key);
    return { result: entry ? entry.value : null };
  };
  const scan = async (from, limit) => {
    const table = await loadTable();
    return { next: null, result: table.map(e => e.value) };
  };
  const set = async (key, value) => {
    const table = await loadTable();
